...
import { listPendingLeaveRequests, listApprovedLeaveRequests, computeDayPeriodDays } from '@/lib/leaveRequests'
import { ReviewLeaveRow } from './ReviewLeaveRow'
import { CancelApprovedLeaveButton } from './CancelApprovedLeaveButton'
...
export default async function AdminLeaveReviewPage() {
  ...
  const [requests, approvedRequests] = await Promise.all([
    listPendingLeaveRequests(supabase),
    listApprovedLeaveRequests(supabase),
  ])
  ...
  return (
    <>
      ...
      <section className={`${card} mt-6 p-0`}>
        <h2 className="px-6 pt-6 font-display text-base font-semibold text-ink">Approved leave (cancel if needed)</h2>
        {approvedRequests.length === 0 ? (
          <p className="p-6 text-[0.9375rem] text-ink-muted">No approved leave requests.</p>
        ) : (
              ...
              <tbody>
                {approvedRequests.map((r) => (
                  <tr key={r.id} className="border-b border-border align-top last:border-0">
                    ...
                    <td className="px-6 py-4">
                      <CancelApprovedLeaveButton requestId={r.id} />
                    </td>
                  </tr>
                ))}
              </tbody>
