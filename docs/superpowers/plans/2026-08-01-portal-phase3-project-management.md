...
-- is_manager_of() in their qual, which is not otherwise apparent from this
-- migration's other statements — is_manager_of(uuid) cannot be dropped
-- while either policy still depends on it (Postgres error 2BP01, the same
-- class of ordering bug already caught once in Task 11's migration).
...
-- visibility, not something else relies on — the remaining is_admin() and
-- assigned_to=self clauses are unchanged from the original.
